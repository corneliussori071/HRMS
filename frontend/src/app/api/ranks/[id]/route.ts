...
import { updateRankSchema } from "@/lib/validations/department-config";
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  ...
  if (!idResult.success) return notFoundResponse("Rank");
  ...
  if (!data) return notFoundResponse("Rank");
...
export async function DELETE(_request: NextRequest, { params }: RouteParams) {
  ...
  if (error) return errorResponse("Failed to delete rank", 500);
